...
function makeKimiAdapter(options?: KimiAdapterLiveOptions) {
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const kimiModelSelection =
            input.modelSelection?.provider === "kimi" ? input.modelSelection : undefined;
    ...
    const respondToUserInput: KimiAdapterShape["respondToUserInput"] = (threadId, requestId) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: `Kimi provider does not support user-input requests (unknown request: ${requestId}).`,
        });
      });
...
export function makeKimiAdapterLive(opts?: KimiAdapterLiveOptions) {
  return Layer.effect(KimiAdapter, makeKimiAdapter(opts));
}
